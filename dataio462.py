...
def get_input_output_dist(pdb_id_list, features_path, distmap_path, pad_size, OUTL, expected_n_channels = 231+231):
    XX = np.full((len(pdb_id_list), OUTL, OUTL, expected_n_channels), 0.0)
    YY = np.full((len(pdb_id_list), OUTL, OUTL, 1), 100.0)
    for i, pdb in enumerate(pdb_id_list):
        X = get_feature(pdb, features_path, expected_n_channels)
        assert len(X[0, 0, :]) == 231+231
...
def get_feature(pdb, features_path, expected_n_channels):
    xpre = np.load(features_path + '/pre231/' + pdb + '.pre231.npy')
    l1 = len(xpre[0, :, 0, 0])
    pre = xpre.reshape(l1, l1, 231)
    assert len(pre[0, 0, :]) == 231

    xcov = np.load(features_path + '/cov231/' + pdb + '.cov231.npy')[0]
    avgpre_512_512_231 = np.load('/nvme2tb/nachammai/FeatureExtraction/AVGcov231/250AVG.cov231.npy')
    avgpre_L_L_231 = avgpre_512_512_231[:l1,:l1,:231]
    xcov = avgpre_L_L_231

    assert len(xcov[0, 0, :]) == 231

    x = np.concatenate((xcov, pre), axis = 2)
    assert len(x[0, 0, :]) == expected_n_channels
